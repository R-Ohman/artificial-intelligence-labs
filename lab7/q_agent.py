...
class QAgent(Agent):
    ...
    def __init__(self, n_states, n_actions,
        ...
        self.lr = 0.1                   # współczynnik uczenia (learning rate): [0.01, 0.1]
        self.gamma = 0.99               # współczynnik dyskontowania: [0.9, 0.99]
        self.epsilon = 1                # epsilon (p-wo akcji losowej)
        self.eps_decrement = 2e-5     # wartość, o którą zmniejsza się epsilon po każdym kroku
    ...
    def init_q_table(self, initial_q_value=0.):
        ...
        q_table = np.zeros((self.n_states, len(self.action_space))) + initial_q_value
        return q_table
    ...
    def learn(self, state: State, action: Action, reward: float, new_state: State, done: bool) -> None:
        ...
        self.q_table[state, action] += self.lr * (reward + self.gamma * np.max(self.q_table[new_state, :]) - self.q_table[state, action])   # 7.6
