...
class Connect4:
    ...
    def is_winner(self, token):
        return self.wins == token

    def copy(self):
        connect4_copy = Connect4(width=self.width, height=self.height)
        connect4_copy.who_moves = self.who_moves
        connect4_copy.game_over = self.game_over
        connect4_copy.wins = self.wins
        connect4_copy.board = [row.copy() for row in self.board]
        return connect4_copy
